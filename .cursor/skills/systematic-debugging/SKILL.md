...
For **model-quality** problems — scores that look wrong rather than code that throws — the failing
artifact is an image, not a stack trace. Capture the input image, the debug overlay
(`eye-quality score <image> --debug-dir …`), and the expected-vs-actual number before forming a
hypothesis. A detector that silently returns no keypoints looks identical to a scoring bug from the
outside.
