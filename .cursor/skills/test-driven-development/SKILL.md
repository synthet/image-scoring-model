...
- Fast subset: `python -m pytest -m "not gpu"` — GPU-marked tests need YOLO weights in `models/`
- Narrowest scope while iterating: `python -m pytest tests/test_<area>.py -q`

Scoring changes need a fixture image and an asserted numeric range, not a smoke test — a heuristic
that returns *a* number for every input passes a smoke test and still be wrong.
