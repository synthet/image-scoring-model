...
- You hit an environment quirk (CUDA, weights download, dataset layout, a training flag) and found a
  workaround.
...
| **Mistake** | Wrong path, wrong split, failed command, invented flag | A guardrail or routing line |
| **Repetition** | Same command sequence three or more times | A harness candidate → [`/compile-skill`](../../commands/compile-skill.md) |
| **Workaround** | Environment discovery that cost you a detour (CUDA build, weights fetch, OOM fix) | A concrete command in the relevant skill |
...
1. It would change behavior on a **different, future** task — not just this run or this image.
2. It is stable, not a one-time glitch (a single CUDA OOM at an unusual batch size is not a lesson;
   "this GPU cannot hold batch size 32 at 1280px" is).
3. It is not already written down. Check the asset and `docs/` before claiming the gap.
...
| A fact about this machine, dataset, or a one-off outcome | A note in `docs/` — *not* a skill |
...
for any new or materially changed skill.
