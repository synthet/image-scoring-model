...
- **Evidence or it didn't happen.** "Looks done", "should work", or "probably green" are not
  ...
  running the narrowest relevant test or command (`python -m pytest tests/test_<area>.py -q`,
  `eye-quality score <image>`).
...
|----|-------------------|---------|----------|
| AC-1 | … | Verified | `python -m pytest tests/test_scoring.py -q` passed |
| AC-2 | … | Unknown  | needs GPU weights; `gpu`-marked tests skipped |
