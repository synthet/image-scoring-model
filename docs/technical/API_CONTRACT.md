...
title: API contract
description: Stable JSON output schema for eye-quality score and batch CLI.
resource: technical/API_CONTRACT.md
tags: [docs, api, contract]
...
Stable JSON shape produced by `score_image()` and the `eye-quality` CLI. Backend integration should treat this as the canonical payload (see [BACKEND_INTEGRATION.md](../guides/BACKEND_INTEGRATION.md)).
...
To access the bird box from the pose model, call `PoseLocalizer.predict()` directly. Pad the crop before sending to BioCLIP for species ID (see [BIRD_DETECTION.md](../architecture/BIRD_DETECTION.md#species-identification-with-bioclip)). A future field such as `subject_bbox_norm` may be added to the pipeline result.
